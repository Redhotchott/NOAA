...
for(i in 1:12){
  ...
  train.bal<-c(sample(which(ptype.temp=='IP'),ip.length[[i]]),sample(which(ptype.temp=='FZRA'),300) )
...
for(i in 1:12){
  ip.rows<-bal.train.rows[[i]][1:ip.length[i]]
  fz.rows<-bal.train.rows[[i]][(ip.length[i]+1):length(bal.train.rows[[i]])]
...
for (i in 1:12){
  ...
  if(i!=7){X <- rbind(X,add.noise(X[1:(300-ip.length[i]),]))} # the first 150 are the ip obs
  N <- nrow(X)# number of examples 
  y <- nptype[bal.train.rows[[i]],] # class labels
  if(i!=7){y <- c(y,y[1:(300-ip.length[i])])} # adding in the labels for the add. noisy ip obs
...
saveRDS(noaa.nnet, "noaannet11b.rds")
saveRDS(predicted, "predicted11b.rds")
