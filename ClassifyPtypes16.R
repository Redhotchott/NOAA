...
# to add one hot month to the script
# traininng sets 12 disjoint testing sets.
# Want to be 90.94% accuracy from 10f
# Only temp as input
...
nmonth<-matrix(0,nrow=length(all.months),ncol=9)  ##Setting up my pytpes as a 4 columns indicating ptype
for ( i in 1:length(all.months)){
  
  if (all.months[i]>6) {nmonth[i,(all.months[i]-3)]<-1}
  else {nmonth[i,all.months[i]]<-1}
}
...
for (i in 1:12){
  ...
  Xnmonth<-nmonth[bal.train.rows[[i]],]
  if(i!=7){Xnmonth <-rbind(Xnmonth,Xnmonth[1:(300-ip.length[i]),])}
  X.proc <- cbind(X.proc,Xnmonth)
  ...
  Xcv.proc <- cbind(Xcv.proc, nmonth[bal.cv.rows[[i]],])
  Xt <- Twb.prof[bal.test.rows[[i]],] #cv data
  yt <-nptype[bal.test.rows[[i]],]# class labels
  Xt.proc <- Xt[,]/max(X) # scale test data
  Xt.proc <- cbind(Xt.proc, nmonth[bal.test.rows[[i]],])
  Y <-matrix(0, N, K) 
  for (j in 1:N){
    Y[j, y[j]]<- 1
  }
...
# saveRDS(noaa.nnet, "noaannet16.rds")
# saveRDS(predicted, "predicted16.rds")
# saveRDS(true, "true16.rds")
