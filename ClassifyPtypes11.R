...
######ABOUT THIS SCRIPT#####
# Trying to beat 90.87% accuracy, 10e, 
# which has 150 of each ptype in each 
# training set.
# This script tries to add noise across 
# the 150 chosen training set to create 
# "new observations" that are slightly different. 
# CORRECTLY CLASSIFIED A IP OBS!!!! 90.89% accuracy
# Only temp as input
...
#Finding overall averages and variances for each level. 
ip.rows<-which(ptype=='IP')
means<-apply(Twb.prof[ip.rows,], 2, mean)
vars<-apply(Twb.prof[ip.rows,], 2, var)
stds<-apply(Twb.prof[ip.rows,], 2, sd)
high<-means+qt(.9, length(ip.rows))*stds
low<-means-qt(.9, length(ip.rows))*stds
plot(means, 1:31, xlim=c(257,277), type='l', col='blue', xlab='Temperature', ylab='100 Meters AGL', main='IP PType 90% CI')
lines(high, 1:31, col='red')
lines(low, 1: 31, col='red')

# This function will generate the "Noise" matrix to add to the 
# Twb.prof matrix to effectively create new IP observations
add.noise<-function(my.matrix){
  noisy<-sapply(stds,FUN=rnorm, mean=0,n=nrow(my.matrix))
  return(noisy+my.matrix)
}
...
for(i in 1:12){
  ...
  train.bal<-c(sample(which(ptype.temp=='IP'),150), sample(which(ptype.temp=='SN'),300),sample(which(ptype.temp=='FZRA'),300),sample(which(ptype.temp=='RA'),300) )
...
for (i in 1:12){
  ...
  X <- rbind(X,add.noise(X[1:150,])) # the first 150 are the ip obs
  N <- nrow(X)# number of examples 
  y <- nptype[bal.train.rows[[i]],] # class labels
  y <- c(y,y[1:150]) # adding in the labels for the add. noisy ip obs
...
saveRDS(noaa.nnet, "noaannet11.rds")
saveRDS(predicted, "predicted11.rds")
saveRDS(true, "true11.rds")
